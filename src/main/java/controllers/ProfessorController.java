...
import controllers.EditAppointmentController;
...
public class ProfessorController implements Initializable {
    ...
    @FXML
    private TableView<Consultations> todayTableView;

    @FXML
    private TableView<Consultations> otherDaysTableView;

    @FXML
    private TableColumn<Consultations, String> todayLendaColumn;

    @FXML
    private TableColumn<Consultations, String> todayStudentiColumn;

    @FXML
    private TableColumn<Consultations, Timestamp> todayDataColumn;

    @FXML
    private TableColumn<Consultations, String> otherDaysLendaColumn;

    @FXML
    private TableColumn<Consultations, String> otherDaysStudentiColumn;

    @FXML
    private TableColumn<Consultations, Timestamp> otherDaysDataColumn;
    ...
    private void initDb() throws Exception{
        conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/konsultimet_test", "root", "tiktak123");
    }
    Connection connDB = null;


    private ArrayList<Consultations> getKonsultimet(boolean thisDay) throws Exception{
        LocalDate today = LocalDate.now();
        String strToday = today.toString();
        ArrayList<Consultations> konsultimet = new ArrayList<>();
        ...
        if (thisDay) {
            ...
            while (resultSet.next()) {
                Consultations k = new Consultations(
    ...
    public void onEditButtonClick(ActionEvent e) throws Exception{
        Consultations selected = otherDaysTableView.getSelectionModel().getSelectedItem();
        if(selected == null) return;
        Locale currentLocale=Locale.getDefault();
        Locale locale = new Locale("Al_AL");
        ResourceBundle bundle=ResourceBundle.getBundle("resources.labelText",locale);
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("/application/edit_appointment.fxml"));
    ...
    public void fillTheTables() throws Exception{
        ObservableList<Consultations> itemsToday = FXCollections.observableArrayList(getKonsultimet(true));
        todayTableView.setItems(itemsToday);

        ObservableList<Consultations> items = FXCollections.observableArrayList(getKonsultimet(false));
